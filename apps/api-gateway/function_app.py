...
def list_survey_images(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if theme not in ('', 'light', 'dark'):
        return _create_error_response("theme must be '', 'light', or 'dark'", 400)
    ...
    try:
        container = cache_manager.get_container_client("surveys")
        items: List[Dict[str, Any]] = []
        for blob in container.list_blobs():
            detected_theme = infer_theme(blob.name)
            if theme and detected_theme != theme:
                continue
            items.append({
                "name": blob.name,
                "slug": infer_slug(blob.name),
                "theme": detected_theme,
                "size_bytes": blob.size,
                "last_modified": blob.last_modified.isoformat() if blob.last_modified else None,
            })
        items.sort(key=lambda x: (x['theme'], x['name']))
        return _create_success_response({
            "theme": theme or 'all',
            "count": len(items),
            "items": items,
        ...
        logger.error("Failed to list survey images: %s", exc, exc_info=True)
        return _create_error_response("Failed to list survey images", 500)


@app.route(route="survey-image", methods=["GET"], auth_level=func.AuthLevel.ANONYMOUS)
def get_survey_image(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return _build_image_response(blob_client, req)
    except Exception as exc:
        if ResourceNotFoundError and isinstance(exc, ResourceNotFoundError):
            return _create_error_response("Image not found", 404)
        logger.error("Failed to proxy survey image '%s': %s", blob_path, exc, exc_info=True)
        return _create_error_response("Failed to load image", 500)