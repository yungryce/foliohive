...
def extract_config_content(file_path: str, raw_content: str) -> Optional[dict]:
    """Extract structured config from file content.
    
    Args:
        file_path: Path to config file (used for routing to correct extractor)
        raw_content: Raw file content to extract from
        
    Returns:
        Structured dict output from extractor, or None if no extractor matches file type
    """
    extractor = get_config_extractor(file_path)
    if not extractor:
        return None
    
    return extractor(raw_content)
