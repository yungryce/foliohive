#!/usr/bin/env bash
# Setup development environment for foliohive backend (v0.4.0)
# Creates a single consolidated virtual environment at api/v0.4.0/.venv
...
# v0.4.0 is consolidated into a single Function App hosting multiple blueprints.
...
check_python() {
    ...
    if [[ -n "$REQUESTED_PYTHON_VERSION" && "$REQUESTED_PYTHON_VERSION" != "3.13" ]]; then
        log_warn "Using Python $PYTHON_VERSION."
    fi
