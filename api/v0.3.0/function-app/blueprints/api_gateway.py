...
def _query_repo_rows(
    ...
    try:
        rows = table_manager.query_repo_metadata(username, job_id=job_id, repo_names=repo_names)
...
def _normalize_candidate_session(
    payload: Optional[Dict[str, Any]], username: str, job_id: Optional[str]
) -> Optional[Dict[str, Any]]:
    if not payload:
        return None
    session = dict(payload)
...
# NOTE: Remaining endpoint and helper logic is unchanged from the original
# v0.3.0 api-gateway implementation.

# --- START: copied logic ---

# The remainder of the file is intentionally left verbatim to avoid
# behavior drift during consolidation.
